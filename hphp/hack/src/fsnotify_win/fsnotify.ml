...
(*
   ...
   shared list. This thread also push a single character in a given
   pipe, whenever a new events is inserted in the shared list.

   On the OCaml side, we might wait for new event by passing the pipe
   to `Unix.select`, and then we might recovers events by calling
   `raw_read_events`.
